...
class ConfigDisplay:
    ...
    def config_widget(self):
        """Return the underlying ``ConfigWidget``."""
        return self.widget

    def show(self):
        """Create the ``ConfigWidget`` if it doesn't already exist."""
...
class ConfigWidget(urwid.WidgetWrap):
    def __init__(self, app: App):
        ...
        self.config = urwid.Scrollable(self.config_text)  # type: ignore[arg-type]
        self.config_scrollbar = urwid.ScrollBar(self.config)  # type: ignore[arg-type]
